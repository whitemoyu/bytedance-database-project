...
std::vector<char *> CsvDataLoader::GetCustomRows() {
  std::lock_guard<std::mutex> lock(mtx_);

  // already loaded, just return
  if (nullptr != Custom_rows_) {
    return *Custom_rows_;
  }

  // load from file
  Custom_rows_ = new std::vector<char *>();
  ...
  for (CSVRow &row : reader) {
    char *line = new char[FIXED_CUSTOM_FIELD_LEN * num_cols_];
    auto offset = 0;
    for (CSVField &field : row) {
      auto t = field.get<int16_t>();
      *(int16_t *)(line + offset) = t;
      offset += FIXED_CUSTOM_FIELD_LEN;
    }
    Custom_rows_->emplace_back(line);
  }
  return *Custom_rows_;
}
} // namespace bytedance_db_project
